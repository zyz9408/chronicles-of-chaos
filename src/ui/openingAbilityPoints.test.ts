...
describe('opening ability point allocation', () => {
  ...
  it('returns allocated points on decrease without allowing a score below its selected base', () => {
    ...
    expect(getOpeningAbilityPointsRemaining(base, current, keys)).toBe(23);
    expect(canDecreaseOpeningAbility(base, current, keys, '智力')).toBe(true);

    current = adjustOpeningAbilityAllocation(base, current, keys, '智力', -99);
    expect(current.智力).toBe(52);
