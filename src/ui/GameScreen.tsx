...
import { formatElapsedTime, formatTokenCount, getTurnDisplayTitle } from '../engine/turn/turnDisplay';
...
import { equipInventoryItem, removePlayerItem } from '../engine/character/playerLoadout';
import { allocatePlayerGrowthPoint, CORE_PLAYER_ATTRIBUTE_KEYS } from '../engine/character/progression';
...
import { TRAIT_RARITY_LEGEND_TITLE, buildEquipmentTooltipTitle, buildTraitTooltipTitle, buildUniqueArtTooltipTitle, formatEquipmentQualityLabel, formatKnownSourceLabel, normalizeTraitRarity } from './gameTooltipText';
import {
...
type ActiveTurnPanelMode = 'reasoning' | 'edit' | 'raw';
...
function buildEquipmentRows(equipment: CharacterEquipmentItem[] = []) {
  ...
  return [
    { label: equipmentSlotLabels.weapon, slot: 'weapon' as const, item: equipment.find((item) => item.slot === 'weapon') },
    { label: equipmentSlotLabels.armor, slot: 'armor' as const, item: equipment.find((item) => item.slot === 'armor') },
    { label: equipmentSlotLabels.mount, slot: 'mount' as const, item: equipment.find((item) => item.slot === 'mount') },
...
export const GameScreen: React.FC<Props> = ({
  ...
  const settleExecutionUi = useCallback((
    ...
    options: { clearPendingAction?: boolean } = {},
  ): boolean => {
  ...
  const runTrueOpening = useCallback(async () => {
    ...
    setMessage('正在生成开场剧情...');
    setProcessingStageText('准备开场上下文');
    setNarrativeText('');
    setSuggestedActions([]);
    ...
    try {
      ...
      if (!isTurnExecutionCancelled(error) && isExecutionCurrent(execution)) {
        ...
        setMessage(`真开局失败：${error instanceof Error ? error.message : '开场剧情生成失败'}`);
      }
    } finally {
  ...
  const executeActionFromState = useCallback(async (
    ...
    setIsProcessing(true);
    setMessage('');
    setProcessingStageText('准备回合上下文');
    setNarrativeText('');
    ...
    try {
      ...
      try {
        if (result.encounterStartIntent) {
      ...
      setProcessingStageText('保存回合与快照');
      assertExecutionCurrent(execution);
      ...
      settleExecutionUi(execution, { clearPendingAction: true });
  ...
  const handleSubmit = useCallback(async () => {
    ...
    setPlayerInput('');
    const outcome = await executeActionFromState(runtimeState, actionText);
    if (outcome === 'failed') setPlayerInput(actionText);
  }, [executeActionFromState, isProcessing, playerInput, runtimeState]);
  ...
  const activeTurnReasoningSummary =
    activeTurnEntry?.displayMeta?.reasoningSummary
    || '暂无公开思路摘要。后续真实开局和真实回合会记录生成摘要；这里不会展示模型隐藏推理链。';
  ...
  }, [getLocationName, renderedNarrativeEntries, runtimeState, saveId, worldBook]);
  ...
  const playerProfileModel = buildPlayerProfilePanelModel(p, runtimeState.memoryArchive);
  ...
  const backpackPanelModel = buildBackpackPanelModel(p);
  ...
  const troopHeaderMetaRows = pickTroopRows(['所属势力', '主将']);
  const troopStatRows = pickTroopRows(['兵种', '规模', '精锐度', '整备', '士气', '训练', '补给']);
        ...
        <div className="game-body">
          ...
          >
            <div className="profile-card">
              ...
              <div className="profile-row">
                ...
                <span className="profile-value">Lv.{p.level ?? 1} · 经验 {p.xp ?? 0} · 成长点 {p.growthPoints ?? 0}</span>
              </div>
                ...
                <div className="profile-tags">
                  ...
                  <div className="res-grid">
                    {p.uniqueArts.map((art) => (
                      <span
                        key={art.id}
                        className={`res-chip unique-art rarity-${normalizeTraitRarity(art.rarity)}`}
          ...
          >
                          ...
                          <div className="turn-display-main">
                            ...
                            <details className="turn-inspection-menu" data-testid="turn-inspection-menu">
                              ...
                              <div className="turn-inspection-popover">
                                <button
                                  type="button"
                                  data-testid="turn-reasoning-button"
                                  onClick={() => openTurnPanel('reasoning', entry)}
                                >
                                  思路摘要
                                </button>
                                <button
            ...
            <div className="game-input-footer">
              ...
              {isProcessing && processingStageText && (
                <div className="processing-stage-box" data-testid="processing-stage-box" role="status" aria-live="polite">
                  <span>系统正在：{processingStageText}</span>
                  {narrativeText.trim() && <small>正文已返回，正在完成结构化写回与保存</small>}
                </div>
              )}
              ...
              <div className="input-row">
                ...
                <button
                  ...
                  aria-label={turnSubmitButton.label}
                >
                ...
                {visibleAbilityEntries.length > 0 && (
                  <section className="player-profile-section">
                    <div className="player-profile-section-head">
                      <h4>能力</h4>
                      <span className="player-profile-growth-points">可分配成长点 {growthPointsAvailable}</span>
                    </div>
                    ...
                    {!selectedNpcCard ? (
                      ...
                      <div className="npc-detail-layout">
                        <div className="npc-detail-main">
                            ...
                            <section className="npc-card-section">
                              ...
                              <div className="npc-chip-row">
                                {selectedNpcCard.uniqueArtChips.map((art) => (
                                  <span
                                    key={`${selectedNpcCard.id}-unique-art-${art.label}`}
                                    className={`npc-trait-chip unique-art rarity-${art.rarity}`}
                                    title={art.title}
                                  >
                                    {art.label}
                                  </span>
                                ...
                                {selectedNpcCard.traitChips.map((trait) => (
                                  <span
                                    key={`${selectedNpcCard.id}-trait-${trait.label}`}
                          ...
                          {selectedNpcCard.memoryPreview.length > 0 && (
                            <section className="npc-card-section">
                              <h4>记忆</h4>
                              <div className="npc-memory-list">
                                {selectedNpcCard.memoryPreview.map((memory) => <p key={`${selectedNpcCard.id}-memory-${memory}`}>{memory}</p>)}
                              </div>
                            </section>
                          )}
                        </div>
                      ...
                      return (
                        ...
                        >
                          ...
                          {item.canEquip && targetSlot && (
                            ...
                            >
                              {item.isEquipped ? '已装备' : slotMismatch ? '槽位不符' : equipmentChooserSlot ? '换上' : '装备'}
                            </button>
                          )}
                        </article>
                      );
                    ...
                    <>
                      ...
                      {factionRecentActions.length > 0 && (
                        <div className="strategic-detail-notes">
                          <span>近期动作记录</span>
                          <ul>
                            {factionRecentActions.map((action) => (
                ...
                <section className="strategic-detail" aria-label="领地详情">
                  ...
                  {activeHoldingTab === 'privateAssets' && (
                    <>
                      ...
                      <div className="holding-subsection-title">在建工程</div>
                  ...
                  {activeHoldingTab === 'controlledHoldings' && (selectedHolding ? (
                    <>
                      ...
                      <div className="holding-controlled-layout">
                        <div className="holding-controlled-top-row">
              ...
              <div className="strategic-archive-layout">
                <aside className="strategic-roster" aria-label="绝艺列表">
                  ...
                  {uniqueArtsPanelModel.rosterItems.length === 0 ? (
                    <p className="muted">暂无绝艺。真开局或后续剧情中的习得与成长会记录在这里。</p>
          ...
          <div className="system-modal-backdrop" role="presentation" onClick={() => setActiveTurnPanel(null)}>
            ...
            >
              <SystemModalHeader
                title={(
                  <>
                    {activeTurnPanel.mode === 'reasoning' && '思路摘要'}
              ...
              {activeTurnPanel.mode === 'reasoning' && (
                <div className="turn-inspection-text" data-testid="turn-reasoning-content">
                  {activeTurnReasoningSummary}
                </div>
              )}
