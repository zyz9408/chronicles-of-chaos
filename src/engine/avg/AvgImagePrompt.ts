...
export function finalizeAvgImagePrompt(input: AvgImagePromptDraft & { editedDraft: string; supplement: string; boldNonExplicit: boolean }): string {
  ...
  return [edited, input.boldNonExplicit && input.adultDirectionAvailable ? '美术方向：大胆但不露骨，增强成年角色的视觉吸引力；仍保持完整服装并禁止裸体或露骨内容。' : '', supplement ? `玩家补充要求：${supplement}` : '', input.safetyMode === 'adult-actor' ? ADULT_GUARD : SCENE_GUARD].filter(Boolean).join('\n');
}
