...
function generateStateWriterContext(
  ...
  parts.push('- AVG 的 avg-presentation:/avg-local: ID 仅用于演出，禁止直接用于人物志、关系、记忆、在场、父母或怀孕等世界事实；固定历史人物必须复用唯一 NPC ID，各模块使用同一身份。成人关系只有出生日期与当前日期确认成年后才可宣告和写回；年龄未知或未成年不得先宣告成功再提交必然失败的关系，更不能为通过校验篡改生日。');
