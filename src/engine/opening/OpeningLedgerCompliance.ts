  ...
  return [
    {
      ...
      content: [
        ...
        '军职、统兵、守城、镇守、军营、兵营、武库、库房、军械清点、斥候名册只表示部队或军需上下文，不等于控制领地；除非明确有治理、接掌、辖有、府库交接或民政权责，否则不得补 upsertHoldingLedger。',
    ...
    },
    {
      ...
      content: [
        ...
        '',
        '字段要求：',
        '- upsertPrivateAsset 必须包含 privateAssetId/name/type/ownerScope/status/summary；updatedAt 是引擎管理的技术时间戳，可省略，空值会按当前游戏时间补齐；type 用 estate/farmland/workshop/ranch/shop/ferry/mine/other；ownerScope 用 personal/clan/household/retainer/faction；status 用 active/damaged/occupied/disputed/archived。',
        '- upsertHoldingLedger 必须包含 holdingId/name/type/status/summary/civilAdministrationScope/scaleLevel/agriculture/commerce/population/publicOrder/popularSupport/defense/recruitPotential/armory/horseSupply/updatedAt；households/territorial/mixed 还必须包含 corruption；civilAdministrationScope 只能用 none/households/territorial/mixed；type 只能用 county/commandery/city/fort/pass/camp/estate/port/village/other；status 只能用 controlled/contested/temporary/lost/archived；适用分数使用 0-100，scaleLevel 使用 1-5。',
