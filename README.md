...
[English](README.en.md) · **v1.8.7 兼容定制版** · [本项目在线游玩](https://zyz9408.github.io/chronicles-of-chaos/) · [原作者参考网站](https://cocsg.pages.dev/)
...
- **跨设备云存档**：可使用 Discord 登录，在桌面端和移动端之间上传、下载存档；API 设置是否同步由玩家自行决定。
- **人物书信与远场演化**：玩家可以写信、收信和查看往来记录；重要关系人物会沿自身处境、历史节点与游戏时间继续行动。
- **桌面、移动端与双主题界面**：局势、地图、人物志、羁绊、红颜、背包、绝艺、势力、领地、部队、战事、战斗与回忆等面板会随设备尺寸重排，并提供暗色与亮色主题。

## v1.8.7 兼容升级

原作者公开源码仍为 1.7.24。本项目按参考网站 1.8.5～1.8.7 可观察行为补齐稳定性修复，不声称还原原作者源码。保留本地战斗与玩家权威规则、AVG 人物生成、同类图库固定绑定和严格选图。

- 修复过期恢复任务、人物身份桥接、补档字段丢失与羁绊成员丢失。
- 换装、卸装、移除物品、成长点在保存成功后更新界面；旧回合不能覆盖较新存档。
- 手机文件存储受安全策略限制时重试备用存储；普通 JSON 主 API 响应兼容。
- 回合提交使用存档摘要和相关快照；云端覆盖保护已补入后端源码。
- GitHub Pages 是静态部署，不具备原站 Cloudflare D1/R2 与 Discord 云端服务；不迁移浏览器存档或自动下载资源包。

详见 [升级证据与验收边界](docs/upgrade-v1.8.7.md)。

### 自愈与数值规则定制补丁

“快速自愈”现在按累计游戏时间结算；在“绝艺 → 特质与绝艺数值规则”可查看、绑定规则及实际执行记录。无法识别的复杂设定不会假称已生效。新个人战与战争分别使用 Combat V2.2 / War V2.7，旧检查点维持原规则。数值模拟：`npm run balance:simulate`，详见 [模型、前后结果与边界](docs/ability-and-balance-2026-09-09.md)。

## 既有 v1.8.4 基础功能

- 新增 AVG 沉浸演出、稳定说话人绑定、本地视觉替换与可校验的外置美术资源包。
- 状态写回失败时按依赖域隔离，并保存可验证的恢复证据，不重写已经提交的正文。
- 状态写回、NPC 补档与模拟、世界演化、记忆摘要默认可合并到一次主剧情请求，也可改用独立 API。
- 加强长期事项、NPC 近况、确定性部队疲劳恢复、记忆检索轨迹和 NPC 人物志诊断。
...
项目内置 OpenAI、DeepSeek、Gemini、Anthropic Claude、Qwen、智谱 GLM（含 Coding Plan）、MiniMax、Moonshot/Kimi、豆包、xAI、Groq、Mistral、Ollama、LM Studio、OpenAI-compatible 与自定义接口配置入口。实际模型可用性、计费、内容政策与数据处理方式由玩家选择的第三方服务决定。
...
- 玩家主动启用云存档后，存档压缩包会保存到项目的 Cloudflare R2；登录身份与存档索引保存在 D1。云存档不会自动上传文生图资产，API 设置只有在玩家主动选择时才会单独同步。
